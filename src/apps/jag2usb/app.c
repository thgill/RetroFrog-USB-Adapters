...
static void on_button_event(button_event_t event)
{
    switch (event) {
        ...
        case BUTTON_EVENT_DOUBLE_CLICK: {
            ...
            printf("[app:jag2usb] Double-click - switching USB mode → %s\n",
        ...
        case BUTTON_EVENT_TRIPLE_CLICK:
            ...
            printf("[app:jag2usb] Triple-click - resetting to HID mode...\n");
            if (!usbd_reset_to_hid()) {
                printf("[app:jag2usb] Already in HID mode\n");
            }
            break;
...
void app_init(void)
{
    printf("[app:jag2usb] Initializing JAG2USB v%s\n", JOYPAD_VERSION);

    // Initialize button service (uses BOOTSEL button on Pico W)
    button_init();
    button_set_callback(on_button_event);

    // Configure router for Jaguar -> USB routing
    ...
    // Add route: Native Jaguar -> USB Device
    router_add_route(INPUT_SOURCE_NATIVE_JAGUAR, OUTPUT_TARGET_USB_DEVICE, 0);
    ...
    static const profile_config_t profile_cfg = {
        ...
        .shared_profiles = &jag2usb_profile_set
    };

    profile_init(&profile_cfg);

    printf("[app:jag2usb] Initialization complete\n");
    printf("[app:jag2usb]   Routing: Jaguar → USB HID Gamepad (+numpad keypad)\n");
    printf("[app:jag2usb]   Jaguar pins: J0..J3=%d,%d,%d,%d B0=%d B1=%d J8..J11=%d,%d,%d,%d\n",
